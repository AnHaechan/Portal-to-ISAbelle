...
# if heuristic is true, run `by auto, by simp, by blast, ..., by arith`, total 9 tactics before sledgehammer
...
theorem_names_path = "/home/haechan.an/Portal-to-ISAbelle/universal_test_theorems"
...
for file_name in glob.glob(f"{theorem_names_path}/test_name_*.json", recursive=True):
    ...
    # only test "test_name_0" ~ "test_name_899" for quick test
    if number <= 900:
